...
# ⭐️ 새로운 혁명 파라미터 ⭐️
VOLATILITY_MULTIPLIER = 10  # 변동성 가속기 (가격 변동이 클수록 더 큰 배수)
MOMENTUM_BOOST = 5          # 모멘텀 부스터 (연속 같은 방향 시 가속)
COMPOUND_FACTOR = 1.1       # 복리 효과 (수익이 쌓일수록 포지션 증가)

fee_rate = 0.001
capital = 50
base_position_size = 0.002  # 기본 포지션
n_steps = 1000
sleep_sec = 1
...
def calculate_revolutionary_profit(abs_price_change, current_position, streak, capital_growth):
    ...
    # 1️⃣ 변동성 가속기: 가격 변동이 클수록 추가 배수
    # $0.1 변동 = 1배, $1 변동 = 2배, $10 변동 = 3배
    volatility_boost = 1 + math.log10(max(abs_price_change, 0.1))
    
    # 2️⃣ 모멘텀 부스터: 연속 같은 방향 거래 시 가속
    # 3연승 이상 = 추가 배수
    momentum_multiplier = 1 + (streak * 0.1) if streak >= 3 else 1
    
    # 3️⃣ 복리 효과: 자본이 증가할수록 포지션 증가
    # 초기 $50 → 1배, $100 → 1.5배, $200 → 2배
    ...
    for step in range(n_steps):
        ...
        if abs_price_change > 0:
            ...
            try:
                ...
                print(f"  ✓ 진입: {side} {dynamic_position} BTC (동적 조정됨)", flush=True)
