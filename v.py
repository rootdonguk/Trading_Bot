...
try:
    ...
    # 최소 주문 금액
    min_notional = 100
    min_position_size = round(min_notional / current_price, 3)
    
    print(f"💎 현재 BTC 가격: ${current_price:.2f}", flush=True)
    print(f"📊 최소 포지션: {min_position_size} BTC (${min_notional})", flush=True)
    print(f"🎯 시작 자본: ${capital}", flush=True)
    print(f"🔥 누적 전략: ${ACCUMULATION_THRESHOLD}까지 누적 후 거래!", flush=True)
        ...
        if abs_price_change > 0:
            ...
            if accumulated_amount >= ACCUMULATION_THRESHOLD:
                ...
                try:
                    ...
                    actual_position = round(ACCUMULATION_THRESHOLD / current_price_check, 3)
                    ...
                    print(f"  ✅ 실제 진입: {side} {actual_position} BTC (${actual_position * current_price_check:.2f})", flush=True)
...
### **작동 원리:**
1. **가격이 변할 때마다** → 가상으로 수익 계산 (12중 배수 적용!)
2. **누적 금액이 $100 도달** → 실제 거래 1회 실행!
3. **누적된 모든 수익을 한 번에 획득!**

### **예시:**
```
Step 1: $0.10 변동 → 가상 수익 $5 → 누적: $5
Step 2: $0.20 변동 → 가상 수익 $10 → 누적: $15
Step 3: $0.50 변동 → 가상 수익 $25 → 누적: $40
...
Step 10: $1.00 변동 → 가상 수익 $70 → 누적: $105!
